...
export default function EquipoPage() {
  ...
  async function handleGenerate() {
    ...
    const { error } = await supabase.from("staff_invitations").insert({ tenant_id: profile.tenant_id, code })
  ...
  async function copyCode() {
    if (!showCode) return
    try { await navigator.clipboard.writeText(`${origin}/join?code=${showCode}`); setCopied(true); setTimeout(() => setCopied(false), 2000) } catch { /* ignore */ }
  }
    ...
    <div className="max-w-lg">
      ...
      {showCode && (
        <Card className="mt-4 border-primary">
          <CardHeader><CardTitle className="text-primary">Código generado</CardTitle></CardHeader>
          <CardContent>
            ...
            <Button variant="outline" size="sm" className="mt-3" onClick={() => setShowCode(null)}>Cerrar</Button>
          </CardContent>
        </Card>
      )}
