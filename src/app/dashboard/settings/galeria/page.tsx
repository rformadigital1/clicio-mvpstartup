...
export default function GaleriaPage() {
  ...
  async function loadGallery() {
    ...
    const { data } = await supabase.from("gallery_images").select("*").eq("tenant_id", profile.tenant_id).order("created_at", { ascending: false })
    setImages(data ?? [])
  }
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (uploadErr) { toast({ title: "Error al subir", description: uploadErr.message, variant: "destructive" }); setUploading(false); return }
    const { data: urlData } = supabase.storage.from(STORAGE_BUCKET).getPublicUrl(path)
    const imageUrl = urlData?.publicUrl
    if (!imageUrl) { setUploading(false); return }
    const { error: dbErr } = await supabase.from("gallery_images").insert({ tenant_id: tenantId, image_url: imageUrl })
    if (dbErr) { toast({ title: "Error al guardar", description: dbErr.message, variant: "destructive" }) }
    else { toast({ title: "Imagen agregada" }); loadGallery() }
    setUploading(false)
  }
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold mb-6">Galería de Trabajos</h1>
